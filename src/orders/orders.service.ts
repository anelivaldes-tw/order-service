...
export class OrdersService {
  constructor(
    ...
    @Inject(SEQUELIZE) private readonly sequelize: typeof Order,
  ...
  async create(createOrdersDto: OrderDto): Promise<any> {
    ...
    try {
      ...
      const order = await this.sequelize.transaction(async (t) => {
        const order = await this.orderRepository.create<Order>({ state: "PENDING", ...createOrdersDto }, { transaction: t });
        const outboxOrder = {
          type: order.state,
        ...
        await this.outboxRepository.create<Outbox>(outboxOrder, { transaction: t });
        return order;
      });
